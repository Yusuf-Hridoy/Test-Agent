...
function findings(result: SessionResult, steps: StepRecord[]): string {
  ...
  // Before memory existed there was nothing to compare against, so an
  // un-ingested session keeps the flat Phase 1 list.
  const graded = sorted.filter((f) => f.memory);
  if (!graded.length) {
    return `<h2>Findings</h2>\n${sorted.map((f) => findingCard(f, result, steps)).join("\n")}`;
  }
  const fresh = sorted.filter((f) => f.memory?.status !== "KNOWN");
  const known = sorted.filter((f) => f.memory?.status === "KNOWN");
  return [
    `<h2>New findings (${fresh.length})</h2>`,
    fresh.length
      ? fresh.map((f) => findingCard(f, result, steps)).join("\n")
      : `<div class="card muted">Nothing new — every finding below was already known to this project.</div>`,
    known.length ? `<h2>Known findings (${known.length})</h2>` : "",
    known.map((f) => findingCard(f, result, steps)).join("\n"),
  ].join("\n");
}
...
/** What project memory knows about this defect, if anything (Phase 3 §1.5). */
function memoryChip(f: Finding): string {
  if (!f.memory) return "";
